...
class ManagerTopics(object):
	...
	def set_new_topics_list(self):
		...
		""" 
		self.allSidesScraper = AllSidesScraper()
		topics = self.allSidesScraper.get_curated_topics_from_all_sides()
		...
		for title, url in topics.items():
			...
			topic.description = self.allSidesScraper.get_topic_description(topic)
...
class Topic(object):
	...
	def get_topic_details(self):
		...
		topic_page = requests.get(URL_BASE + self.url)
		tree = html.fromstring(topic_page.text)
		num_pages = allSides.get_num_articles_in_topic(tree)
		self._pagination = num_pages
		infoTable_articles = tree.xpath("//div[contains(@class, 'news-title')]/a")

		for article in infoTable_articles: 
			newArticle = Article(article.text, article.attrib)
	...
	def get_response_data(self):
		...
		for article in self.articles: 
			article_text[article.title] = article.url
